...
# This function will display a Bar Graph of the Top 5 highest or lowest populations of the
# chosen year census.
def displayBarGraph(df, year, top):
    top_5 = df.sort_values(by='Population', ascending=not top).head(5)
    color = 'blue' if  top else 'red'
    ...
    PLT.ylabel('Barangay')
    PLT.title(f'{title} Barangays with {"Highest" if top else "Lowest"} Population in {year} Census')
